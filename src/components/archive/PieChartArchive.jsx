...
const PieChartArchive = ({ data, size = 250 }) => {
    const center = size / 2;
            ...
            return [
                {
                    ...
                    percentage: ((nonZeroItems[0].value / totalValue) * 100).toFixed(1),
    ...
    return (
        <div className="flex flex-col items-center w-full">
            ...
            <div dir='rtl' className="grid grid-cols-2 xl:grid-cols-3 gap-2">
                {data.slice(0, 8).map((item, index) => (
                    <div key={index} className="flex items-center">
                        <div
                            className="w-5 sm:w-6 h-4 mr-2 rounded-sm"
                            style={{ backgroundColor: item.color }}
                        ></div>
                        <span className="text-gray-700 sm:font-semibold px-1">{item.name} ({((item.value / data.reduce((sum, item) => sum + item.value, 0)) * 100).toFixed(1)}%)</span>
                    </div>
                ))}
            </div>
        </div>
...
export default PieChartArchive;
